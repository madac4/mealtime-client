...
const Product: React.FC<IProduct> = ({ product }) => {
    ...
    const handleItemsCount = (id: string) => {
        const item = productsInCart.find((item: any) => item.id === id);
        if (item) {
    ...
    return (
        <div className="p-5 border rounded-md">
            ...
            <p className="mb-2 text-lg font-semibold text-red-600">{product.price} MDL / buc.</p>
            <Button
                className="bg-green-500 w-full mt-auto hover:bg-green-600 active:scale-[0.98] transition-all"
                onClick={handleAddToCart}>
                Adaugă în coș {handleItemsCount(product.id)}
            </Button>
        </div>
    );
};
